...
function resolveDirectory (dir) {
  ...
  const fromFile = readSafe(fileSource)
  ...
  const fromPackage = readSafe(packageSource)
    .then(contents => contents && {
      json5: false,
  ...
  add (config) {
    ...
    if (extendsClause) {
      ...
      if (this.pointers.has(extendsSource)) {
        ...
        const promise = resolveFile(extendsSource)
