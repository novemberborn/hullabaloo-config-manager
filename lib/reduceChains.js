...
function reduceOptions (chain, envName, pluginsAndPresets, dependencies, sources) {
  let json5 = false
...
function reduceChains (chains, cache) {
  ...
  const unflattenedDefaultOptions = reduceOptions(chains.defaultChain, null, pluginsAndPresets, dependencies, sources)
  ...
  for (const pair of chains.envChains) {
    ...
    unflattenedEnvOptions.set(envName, reduceOptions(chain, envName, pluginsAndPresets, dependencies, sources))
  }
