...
class Verifier {
  ...
  verifyCurrentEnv (cache) {
    const envName = currentEnv()

    const sourcesToHash = this.selectByEnv(this.sources, envName)
    const expectedSourceHashes = sourcesToHash.map(item => item.hash)
    const pendingSourceHashes = hashSources(sourcesToHash, cache)

    const babelrcFile = path.join(this.babelrcDir, '.babelrc')
    const checkedBabelrcFile = sourcesToHash.some(item => item.source === babelrcFile)
      ? true
      : ensureMissingBabelrcFile(babelrcFile, cache)
  ...
  static hashAndCreate (babelrcDir, envNames, dependencies, sources, cache) {
    return Promise.all([
      hashDependencies(dependencies, cache),
      hashSources(sources, cache)
    ])
      .then(results => {
